...
extern const double Minimum_Shape_Quality_Swap = 0.65;     // 网格单元质量的极值，质量小于该值的网格单元与相邻单元运行边交换
extern const double Minimum_Shape_Quality_Smoothing = 0.9; // 网格单元质量的极值，质量小于该值的网格单元运行节点光顺
extern const double Max_Ratio = 2;                         // 1.53;                       // 网格中每条边长度都需要满足与边中点理想长度尺寸值比值小于Max_Ratio，若过长，则需要将该边插点二分
extern const double Delta = 1e-5;                          // 1e-6;                           // 作为函数求导时的Delta使用，和一些判断时的误差消除
...
bool ELEM::Sort_judge()
{
  if (form[0] <= form[1] && form[1] <= form[2])
    return true;
  else
    return false;
}
...
{
  // 读入6个节点编号
  int arr[] = {elem_tp1.get_form(0), elem_tp1.get_form(1), elem_tp1.get_form(2), elem_tp2.get_form(0), elem_tp2.get_form(1), elem_tp2.get_form(2)};
...
// 给定一条边的两个节点，返回该边长度
double get_EdgeLength(NODE node_tp1, NODE node_tp2)
{
  return getDist(node_tp1.get_pos(), node_tp2.get_pos());
}
...
// 空腔查找
void FindElemCav(std::vector<ELEM> *elem, int *elem_num, std::vector<NODE> *node, int *node_num, std::vector<int> *elemNum_Cav, NODE node_Insert, int elemNum_IncludeNodeIns)
{
...
// 空腔修复
void RepairCavity(std::vector<ELEM> *elem, int *elem_num, std::vector<NODE> *node, int *node_num, std::vector<int> *elemNum_Cav, NODE node_Insert)
{
...
// 内部点生成
void CreateFieldNodes(std::vector<ELEM> *elem, int *elem_num, std::vector<NODE> *node, int *node_num, std::vector<NODE> *node_Insert, double *density_control, std::vector<int> *elemNum_IncludeNodeInitial)
{
  ...
  for (std::vector<ELEM>::iterator elem_iter = elem->begin(); elem_iter != elem->end(); ++elem_iter)
  {
    ...
    node_tp.tr_pos(0, (node->at(elem_iter->get_form(0)).get_pos(0) + node->at(elem_iter->get_form(1)).get_pos(0) + node->at(elem_iter->get_form(2)).get_pos(0)) / 3);
    node_tp.tr_pos(1, (node->at(elem_iter->get_form(0)).get_pos(1) + node->at(elem_iter->get_form(1)).get_pos(1) + node->at(elem_iter->get_form(2)).get_pos(1)) / 3);
...
// 内部点插入，适用于一般插入节点
bool InsertFieldPoint(std::vector<ELEM> *elem, int *elem_num, std::vector<NODE> *node, int *node_num, NODE node_Insert, int elemNum_IncludeNodeIns)
{
...
// 在node容器内删除节点与单元
void Removal_NodeElem(std::vector<ELEM> *elem, int *elem_num, std::vector<NODE> *node, int *node_num, std::vector<int> *elemNum_Remove, int *nodeNum_Remove)
{
...
void Positivity_Hessian_Matrix(double hessian_matrix[2][2])
{
  ...
  //   hessian_matrix[1][1] += Weights;
  // }
  if (hessian_matrix[0][0] * hessian_matrix[1][1] <= hessian_matrix[0][1] * hessian_matrix[1][0])
  {
...
// 内部点插入，适用于二分边时插入节点
bool InsertFieldPoint(std::vector<ELEM> *elem, int *elem_num, std::vector<NODE> *node, int *node_num, NODE node_Insert, EDGE edge_operate)
{
  ELEM elem_tp;                 // 声明一个用于临时储存生成网格单元的变量
  EDGE edge_tp;                 // 创建一个结构体，用于临时储存生成的边
  std::vector<int> elemNum_Cav; // 创建一个空腔容器，用来储存包含边edge_operate的网格单元编号
  // 空腔查找
  FindShell(elem, node, edge_operate, &elemNum_Cav, "fast");
  std::vector<EDGE> edge_Cav; // 创建一个容器，用来储存空腔边界，一个边由两个节点构成
  /**
   * * 在edge_Cav内查找当前搜索的网格单元边界，如果搜到end()，代表没有找到，该边是空腔边，可以插入；
   * * 如果没有搜到搜到end()，代表该边不是空腔边，不予插入，并且在edge_Cav内删除该边
  ...
  for (unsigned long long i = 0; i < edge_Cav.size(); i++)
  {
    ...
    if (i < elemNum_Cav.size())
    {
      ...
      Update_Djacency(elem, node, edge_Cav.at(i), "slow"); // 查找包含该边的所有网格单元，并更新相邻关系
      // 修改最新生成网格的质量
      elem->at(elemNum_tp).tr_quality(abs(Shape_Quality(node->at(elem->at(elemNum_tp).get_form(0)), node->at(elem->at(elemNum_tp).get_form(1)), node_Insert)));
    }
    // 之后生成的网格全压入elem中，并赋予新的编号
    else
    {
      ...
      elem->push_back(elem_tp); // 将elem_tp1压入elem中，形成网格单元的生成
      // 修改最新生成网格的质量
      elem->at(*elem_num).tr_quality(abs(Shape_Quality(node->at(elem->at(*elem_num).get_form(0)), node->at(elem->at(*elem_num).get_form(1)), node_Insert)));
      *elem_num += 1;
      Update_Djacency(elem, node, edge_Cav.at(i), "slow"); // 查找包含该边的所有网格单元，并更新相邻关系
    }
  }
...
double cubic_interpolation_step(std::vector<NODE> *node, NODE node_tp, std::vector<EDGE> elemNum_Optimization_Domain, double steepest_descent[2][1], double step_LowerLimit, double step_UpperLimit)
{
...
// 在包含最优解的区间内进一步搜索最优解
double Zoom(std::vector<NODE> *node, NODE node_tp, std::vector<EDGE> elemNum_Optimization_Domain, double steepest_descent[2][1], double step_LowerLimit, double step_UpperLimit)
{
...
double Line_Search_Algorithm_with_Wolfe(std::vector<NODE> *node, NODE node_tp, std::vector<EDGE> elemNum_Optimization_Domain, double steepest_descent[2][1], double Init_step)
{
...
void Construct_Local_Optimization_Domain(std::vector<ELEM> *elem, std::vector<NODE> *node, int borderNode_num, std::vector<int> Init_inferior_elem, std::vector<int> *Laplacian_Smoothing_nodeNum)
{
...
void Construct_Local_Optimization_Domain_All(std::vector<ELEM> *elem, std::vector<NODE> *node, int borderNode_num, std::vector<int> Init_inferior_elem, std::vector<int> *Laplacian_Smoothing_nodeNum)
{
