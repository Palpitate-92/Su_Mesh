 ...
 * * 2：圆形
 * * 3：多边形
 * * 4：直线（一般是位于上述几种图形内部，拥有单独的密度控制信息，作为线加密来使用）
 * * 5：单点
 *
 * * 输入信息时，首先输入外边界网格类型与网格信息，其次输入内部边界网格类型与信息，具体输入信息如下：
 * * 外边界矩形：1 x y xL yL perface scale。图形种类(1)，矩形形心坐标(x,y)，矩形x、y方向上长度，每条边平分为perface个线段，点密度信息比例（边线段间距的scale倍）
 * * 外边界圆形：2 x y r perface scale。图形种类(2)，圆心坐标(x,y)，圆形半径(r)，每1/4个圆弧分为perface个线段，点密度信息比例（边线段间距的scale倍）
 * * 内边界矩形：1 x y xL yL Ld Hd xO yO。图形种类(1)，矩形形心坐标(x,y)，矩形x、y方向上长度(xL,yL)，低密度信息Ld，高密度信息Hd。
 ...
 * * 单点：5 x y Ld Hd rO。单点种类(5)，单点坐标(x,y)，低密度信息Ld，高密度信息Hd，半径(rO)标定一个外部圆形包围该点，且密度由单点处高密度Hd过渡到外部圆形低密度Ld
...
constexpr auto SquareNode_perface = 20;                         // 每条边平分为SquareNode_perface份
constexpr auto SquareFaceLong = 2.0;                            // 正方形外边界长度
...
constexpr auto radius = 0.3;                            // 声明圆半径
constexpr auto RoundNode_perface = 15;                  // 每1/4个圆弧分为RoundNode_perface份
...
// ? 该图形为内边界，需要一些其他信息
constexpr auto OutRound_Radius = radius + 0.3;     // 外部圆形半径
constexpr auto Round_LDensity = Square_spac * 0.8; // 低密度信息

// 圆形内边界2
// constexpr auto Pi = 3.14159265358979;                   // 声明圆周率
constexpr auto center_x_2 = 1.5;                            // 声明圆心X坐标
constexpr auto center_y_2 = 1.5;                            // 声明圆心Y坐标
constexpr auto radius_2 = 0.2;                              // 声明圆半径
...
// ? 该图形为内边界，需要一些其他信息
constexpr auto OutRound_Radius_2 = radius_2 + 0.2; // 外部圆形半径
constexpr auto Round_LDensity_2 = Square_spac;     // 低密度信息

// 单点
constexpr auto point_x = 1.5;                      // 声明单点x坐标
constexpr auto point_y = 1.5;                      // 声明单点y坐标
constexpr auto point_HDensity = Square_spac * 0.2; // 低密度信息
constexpr auto point_LDensity = Square_spac;       // 低密度信息
constexpr auto point_OutRound_Radius = 0.35;        // 外部圆形半径

// 总节点数
constexpr auto node_num = Square_num + Round_num + 1; //+ Round_num_2; // 总节点数
...
int main()
{
  ...
  else
  {
    ...
    outfile << 5 << ' ' << point_x << ' ' << point_y << ' ' << point_LDensity << ' '
            << point_HDensity << ' ' << point_OutRound_Radius << '\n';
    /*
    outfile << node_num << ' ' << 1 << ' ' << Square_num << ' ' << 2 << ' ' << Round_num << ' '
            << 2 << ' ' << Round_num_2 << ' ' << -1 << '\n';
    // 输出密度控制信息
    ...
    */
    ...
    outfile << cnt << ' ' << point_x << ' ' << point_y << ' ' << point_HDensity << '\n';
    /*
    // 输出圆形内边界2
