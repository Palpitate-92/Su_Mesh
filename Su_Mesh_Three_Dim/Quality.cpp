...
std::string
_QUALITY::Check_Sliver_type(NODE node_A, NODE node_B, NODE node_C, NODE node_D)
{
  _DATA_PROCESS data_process;
...
double _QUALITY::target_Function(_SU_MESH *su_mesh, NODE node_tp, std::vector<FACE> face_Optimization_Domain)
{
  ...
  for (std::vector<FACE>::iterator face_iter = face_Optimization_Domain.begin();
       ...
       ++face_iter)
  {
    value_tp = error_Function(su_mesh->node.at(face_iter->form[0]),
                              su_mesh->node.at(face_iter->form[1]),
                              su_mesh->node.at(face_iter->form[2]),
                              node_tp);
    if (value_tp < 0)
      return -1;
...
double _QUALITY::target_Function_h2_second(
    _SU_MESH *su_mesh, NODE node_tp, std::vector<FACE> face_Optimization_Domain, double delta_Value, NODE node_Delta_1, NODE node_Delta_2)
{
...
double _QUALITY::target_Function_h4_second(
    _SU_MESH *su_mesh, NODE node_tp, std::vector<FACE> face_Optimization_Domain, double delta_Value, NODE node_Delta_1, NODE node_Delta_2)
{
  return -1;
}
...
double _QUALITY::step_helper_function_prime(
    _SU_MESH *su_mesh, NODE node_tp, std::vector<FACE> face_Optimization_Domain, double steepest_descent[3][1], double step)
{
  _DATA_PROCESS data_process;
...
double _QUALITY::Zoom(_SU_MESH *su_mesh, NODE node_tp, std::vector<FACE> face_Optimization_Domain, double steepest_descent[3][1], double step_LowerLimit, double step_UpperLimit)
{
  ...
  double
      step_Low_helper_function;   // 定义两个变量，作为总是满足Zoom函数条件的两个数
  double step_High_helper_function;
  double step_current;            // 声明一个变量，作为当前迭代时的步长
  double
      helper_function_LowStep;    // 声明一个变量，储存步长为step_Low_helper_function的辅助函数值
  double helper_function_current; // 声明一个变量，储存本次迭代辅助函数值
  double
      helper_function_primeValue; // 声明一个变量，储存本次迭代辅助函数的导数值
  // 定义一个变量，储存step=0时的辅助函数值
  double helper_function_step0 = step_helper_function(
      su_mesh, node_tp, face_Optimization_Domain, steepest_descent, 0);
  // 定义一个变量，储存step=0时的辅助函数导数值
  double helper_function_step0_prime = step_helper_function_prime(
      su_mesh, node_tp, face_Optimization_Domain, steepest_descent, 0);
  ...
  while (true)
  {
    ...
    step_current = cubic_interpolation_step(
        su_mesh, node_tp, face_Optimization_Domain, steepest_descent, step_Low_helper_function, step_High_helper_function);
    // 计算当前步长下辅助函数值
...
double _QUALITY::Line_Search_Algorithm_with_Wolfe(
    _SU_MESH *su_mesh, NODE node_tp, std::vector<FACE> face_Optimization_Domain, double steepest_descent[3][1], double Init_step)
{
  ...
  while (true)
  {
    ...
    step_current = cubic_interpolation_step(
        su_mesh, node_tp, face_Optimization_Domain, steepest_descent, step_current, step_UpperLimit);
...
void _QUALITY::Construct_Local_Optimization_Domain_All(
    _SU_MESH *su_mesh, std::vector<int> Init_inferior_elem, std::vector<int> *Laplacian_Smoothing_nodeNum)
{
...
bool _QUALITY::Optimization_based_Smoothing_The_all(_SU_MESH *su_mesh, int Optimization_based_Smoothing_nodeNum)
{
  ...
  do
  {
    ...
    if (target_function_value_after != 0)
      target_function_value_before = target_function_value_after;
    // 先由当前点位置求目标函数当前位置梯度
    for (int i = 0; i < 3; i++)
    ...
    // if (Optimization_based_Smoothing_nodeNum == 339 && iter == 4)
    // {
    //   double dis = 10;
    //   std::fstream outfile;                                        //
    //   声明输出文件流 outfile.open("data.txt", std::ios::out |
    //   std::ios::trunc);   // 创建或者打开输出文件，如果此文件已经存在,
    //   则打开文件之前把文件长度截断为0 std::fstream outfile1; //
    //   声明输出文件流 outfile1.open("data1.txt", std::ios::out |
    //   std::ios::trunc); // 创建或者打开输出文件，如果此文件已经存在,
    //   则打开文件之前把文件长度截断为0 for (double i = -dis; i < dis; i +=
    //   dis / 2000)
    //   {
    //     outfile << std::setprecision(16) << i << '\n';
    //     outfile1 << std::setprecision(16) <<
    //     step_helper_function(su_mesh, node_before,
    //     face_Optimization_Domain, steepest_descent, i) << '\n';
    //   }
    //   outfile.close();
    //   outfile1.close();
    //   exit(-1);
...
void _QUALITY::Quality_Information(_SU_MESH *su_mesh)
{
  ...
  int cnt = 0;
  for (std::vector<double>::iterator iter = Shape_Quality.begin();
       iter != Shape_Quality.end();
       ++iter)
  {
...
void _QUALITY::Quality_Optimization_Smoothing(_SU_MESH *su_mesh)
{
  ...
  _MESH_PROCESS mesh_process;
  for (std::vector<int>::iterator nodeNum_iter = Laplacian_Smoothing_nodeNum.begin();
  ...
  for (std::vector<int>::iterator iter = Optimization_based_Smoothing_nodeNum.begin();
