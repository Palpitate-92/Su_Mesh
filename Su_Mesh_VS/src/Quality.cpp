...
void _QUALITY::Construct_Local_Optimization_Domain_All(_SU_MESH *su_mesh, std::vector<int> Init_inferior_elem, std::vector<int> *Laplacian_Smoothing_nodeNum)
{
         ...
         ++iter)
    {
        ...
        for (int i = 0; i < DIM + 1; i++)
        {
            if (su_mesh->elem.at(*iter).form[i] < su_mesh->boundary_recovery_node_num) // 跳过网格边界点
                continue;
...
void _QUALITY::Quality_Optimization_Face_Transform(_SU_MESH *su_mesh)
{
    ...
    while (!elemNum_judge.empty())
    {
        ...
        for (int i = 0; i < DIM + 1; i++)
            ...
            {
                // 跳过边界边
                if (elem_tp.form[i] < su_mesh->boundary_recovery_node_num && elem_tp.form[j] < su_mesh->boundary_recovery_node_num)
                {
                    min_quality[cnt] = -1;
        ...
        for (int i = 0; i < DIM + 1; i++)
            for (int j = i + 1; j < DIM + 1; j++)
                ...
                {
                    // 跳过边界面
                    if (elem_tp.form[i] < su_mesh->boundary_recovery_node_num && elem_tp.form[j] < su_mesh->boundary_recovery_node_num && elem_tp.form[k] < su_mesh->boundary_recovery_node_num)
                    {
                        min_quality[cnt] = -1;
                        face_transform_type[cnt++] = 1;
                        continue;
