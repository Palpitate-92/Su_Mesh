...
int _INSERT_POINT::Insert_point(_SU_MESH* su_mesh, int nodeNum_Last_succ, int nodeNum_Insert, NODE node_Insert, int elemNum_Basis)
{
	...
	if (elemNum_Basis == -1)
	{
		int elemNum_tp = mesh_process.Find_OneElem_IncludeNode_nodeNum(su_mesh, nodeNum_Last_succ, node_Insert);
		if (elemNum_tp == -1)
		{
			std::cout << "Boundary point lookup failed, and the number of the current inserted boundary point is "
				<< nodeNum_Insert << std::endl;
			exit(-1);
		}
		else if (elemNum_tp == -2)
			return 2;
		elemNum_Cavity.push_back(elemNum_tp);
	}
	// 如果是在插入内部点，则直接压入初始的外接圆包含待插入节点的网格单元编号到elemNum_Cavity
	else
	 ...
	 * * 如果没有搜到搜到end()，代表该面不是空腔边界面，不予插入，并且在face_Cavity内删除该网格面
	 */
