...
void _INTERIOR_POINT::CreateFieldNodes(_SU_MESH* su_mesh, std::vector<NODE>* node_Insert, std::vector<int>* elemNum_Basis_Initial)
{
	...
	for (std::vector<ELEM>::iterator elem_iter = su_mesh->elem.begin(); elem_iter != su_mesh->elem.end(); ++elem_iter)
	{
		...
		// }
		// 计算当前网格单元重心
		if (elem_iter->form[0] == -1)continue;
...
void _INTERIOR_POINT::AutoRefine(_SU_MESH* su_mesh)
{
	...
	do
	{
		...
		while (!node_Insert.empty()) // 如果待插入节点容器不为空，则开始插入节点
		{
			...
			{
				// 声明一个变量，用来储存任意一个外接球包含待插入节点的网格单元编号，作为基单元来查找空腔
				int elemNum_Basis;
				// 查找一个外接球包含包含待插入节点的网格单元编号
				elemNum_Basis = mesh_process.Find_OneElem_IncludeNode_elemNum(su_mesh, elemNum_Basis_Initial.at(cnt), *node_iter);
				// 如果该值为-1，基单元查找失败
				if (elemNum_Basis == -1)
				{
					std::cout << "Base unit lookup failed!" << std::endl;
					exit(-1);
				}
				...
				judgment = insert_point.Insert_point(su_mesh, su_mesh->node_num - 1, su_mesh->node_num, *node_iter, elemNum_Basis);
				// 节点插入成功
				if (judgment == 1)
				{
