...
void _BOUNDARY_POINT::IniDelaunay(_SU_MESH* su_mesh)
{
	...
	double density = su_mesh->longest_border_edge * 0.1 + su_mesh->shortest_border_edge * 0.9;
	for (std::vector<NODE>::iterator node_iter = su_mesh->node.begin(); node_iter != su_mesh->node.end(); node_iter++)
...
void _BOUNDARY_POINT::Insert_BoundaryPoint(_SU_MESH* su_mesh)
{
	...
	while (!su_mesh->nodeNum_degradation.empty())
	{
		...
		{
			if (insert_Point.Insert_point(su_mesh, *iter - 1, *iter, su_mesh->node.at(*iter), -1) == 2)
				++iter;
			else
			{
	...
	NODE node_before;
	double disturbance_delta = su_mesh->shortest_border_edge / 100000; // 储存点扰动距离，定义为最短边界边长度的1/100000
