...
std::vector<Pathl> _BOUNDARY_RECOVERY::FindPath(_SU_MESH* su_mesh, EDGE edge_recovery)
{
	...
	for (std::vector<int>::iterator iter = elemNum_IncludeNode.begin(); iter != elemNum_IncludeNode.end(); ++iter)
	{
		...
		pathl_tp.type[0] = 1;
		pathl_tp.pot[0] = su_mesh->node.at(edge_recovery.form[0]);
	...
	while (!flag)
	{
		...
		for (std::vector<Pathl>::iterator iter = path_tp1.begin(); iter != path_tp1.end(); ++iter)
		{
			...
			// 若第一个相交图形是点，则需要判断当前路径元内与该点相对的3条网格边与1个网格面跟待恢复边界边的相交情况
			else if (iter->type[0] == 1)
			{
				if (data_process.Edge_Elem_Intersection(su_mesh->node.at(edge_recovery.form[0]), su_mesh->node.at(edge_recovery.form[1]),
				...
				for (int i = 0; i < 2; i++)
				{
					for (int j = i + 1; j < 3; j++)
					{
						...
						if (data_process.Edge_Edge_Intersection(&intersection_point, su_mesh->node.at(edge_recovery.form[0]), su_mesh->node.at(edge_recovery.form[1]),
				...
				if (!intersection_judge)
					if (data_process.Edge_Face_Intersection(&intersection_point, su_mesh->node.at(edge_recovery.form[0]), su_mesh->node.at(edge_recovery.form[1]),
			...
			// 若第二个相交图形是面，则将当前三角化内与当前路径元相邻且相邻面时该面的路径元直接压入path_tp2
			else
			{
